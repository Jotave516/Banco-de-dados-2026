...
-- 5. Liste o nome e a CH disponível dos professores que possuem CH disponível cadastrada, exibindo as colunas como Professor e Horas_Disponiveis. Ordene o resultado pela CH disponível do maior para o menor.
...
-- Utilize os aliases Professor, Salario e Formacao, ordene pelo salário do maior para o menor e apresente somente os 3 primeiros resultados.
	select nome as 'Professor', salario  as 'Salario', formação as 'Formação' from professor where salario between 5500 and 7500 order by salario desc limit 3;
...
-- não possuem formação em Administração.
	select * from professor where CH_disponivel between 10 and 25 and salario>5500 and formação <> 'Administração';

-- 9. Liste as disciplinas que possuem mais alunos matriculados do que a disciplina "Multimídia". Apresente o nome e a quantidade de matriculados, ordenando do maior para o menor.
	select nome, qtde_matriculados from disciplina where qtde_matriculados >
    (select qtde_matriculados from disciplina where nome='Multimídia') order by qtde_matriculados desc;
    
-- 10. Liste as disciplinas que não possuem professor cadastrado e que não aconteçam nas segundas nem nas quartas.
	select * from disciplina where dia_semana<>'segunda-feira' and dia_semana<>'quarta-feira' and mat_professor is not null;
...
-- 12. Liste os professores que não são coordenadores de nenhum curso. Apresente nome, formação e salário, ordenando pelo nome em ordem alfabética.
	select nome, formação, salario from professor where matricula not in 
...
-- Apresente: nome da disciplina, Alunos, Carga_Horaria. Ordene primeiro pela carga horária em ordem decrescente e, em caso de empate, pela quantidade de alunos em ordem decrescente.
	select nome, qtde_matriculados, CH from disciplina where mat_professor is not null and mat_professor in 
		(select matricula from professor where salario>5800) and
			nome not like '%TCC%' and CH between 40 and 80 
				order by ch desc, qtde_matriculados desc;
